...
import { appliedDecisions, clicksAround, positionAround, impressionsAround, upsertPattern } from "../orchestrator/lib/supabase.mjs";
...
// ADR-006 #1: impressions joined the blend; clicks stay dominant. Weights sum to 1.0.
const CLICK_W = Number(process.env.ATTR_CLICK_WEIGHT || 0.6);
const IMPR_W  = Number(process.env.ATTR_IMPRESSION_WEIGHT || 0.1);
...
async function main() {
  await attribute({
    ...
    upsertPattern,
    log: (m) => console.log(m),
    weights: { click: CLICK_W, impression: IMPR_W, position: POS_W },
    lagDays: LAG,
    minN: MIN_N,
  });
}
