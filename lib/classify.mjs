...
// Returns { rows, usage:{input_tokens,output_tokens} } — usage lets callers report real cost.
...
export async function classifyDomains(
  ...
  if (!items || !items.length) return { rows: [], usage: { input_tokens: 0, output_tokens: 0 } };
  ...
  return { rows: parseClassification(text), usage };
}
