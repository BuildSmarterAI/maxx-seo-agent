#!/usr/bin/env node
// packs/wordpress/update-drafts.mjs
// Updates EXISTING WP posts from drafts/*.md — patches content + Yoast meta.
// Skips slugs that don't exist yet (use wp:publish-drafts for those).
// OVERWRITES current WP post content — run against staging first.
// env: WP_BASE_URL, WP_USER, WP_APP_PASSWORD, SEO_PLUGIN=yoast|rankmath
//
// Usage:
//   npm run wp:update-drafts                         — update all slugs that exist in WP
//   npm run wp:update-drafts cost-per-square-foot-*  — (future: slug filter via argv)
...
const META_KEYS = {
  yoast:    { title: "_yoast_wpseo_title", desc: "_yoast_wpseo_metadesc", canonical: "_yoast_wpseo_canonical" },
  rankmath: { title: "rank_math_title",    desc: "rank_math_description", canonical: "rank_math_canonical_url" },
}[PLUGIN];
...
async function main() {
  ...
  for (const file of files) {
    ...
    if (!existing) {
      console.log(`  - no post   ${slug}  (run wp:publish-drafts to create)`);
      skipped++;
      continue;
    }

    try {
      await wp(`posts/${existing.id}`, {
      ...
      console.log(`✓ updated   id=${existing.id}  /${slug}/  → ${BASE}/?p=${existing.id}&preview=true`);
      updated++;
    } catch (e) {
  ...
  if (updated > 0) console.log(`Review: ${BASE}/wp-admin/edit.php?post_status=draft&post_type=post`);
}
