# ADR-010 — Escalation mirror to Linear

**Date:** 2026-06-25 (recorded 2026-07-10)
**Status:** Accepted — implemented in `scripts/push-escalations.mjs`
**Deciders:** Harris / BuildSmarter

> **Numbering note.** This decision was originally drafted as "ADR-007" on a working branch
> that was never merged. `ADR-007` was subsequently taken by
> [ADR-007 — AI Overview citation intelligence](./ADR-007-ai-overview-citation-intelligence.md).
> The decision is recorded here under the next free number. `ADR-009`'s R3 cross-reference
> has been repointed to this file.
...
- Closing escalated items is manual. If escalation volume grows, a two-way close-sync
  (Linear webhook → `work_queue` done) is the natural follow-up — ranked as R3 in
  [ADR-009](./ADR-009-build-order-reconciliation-survivors.md), and still unbuilt.
...
- `.github/workflows/seo-learn.yml` — runs the mirror after attribution, with
  `LINEAR_API_KEY` from repo secrets
